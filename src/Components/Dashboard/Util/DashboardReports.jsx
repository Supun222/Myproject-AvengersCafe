...
export default function DashboardReports(reportType) {
    ...
    const selectReportType = () =>{
        switch (Number(reportType.reportType)) {
            case ACT_TYPE.BILLD : 
                return setParams({icon :process.env.PUBLIC_URL + `/images/order.png` , name : "Billd", amount :"300"});
            case ACT_TYPE.PENDING : 
                return setParams({ icon : process.env.PUBLIC_URL + `/images/pending.png` , name : "Pending", amount : "300"});
            case ACT_TYPE.COMPLETED : 
                return setParams({icon : process.env.PUBLIC_URL + `/images/dispatch.png` , name : "Dispatch", amount : "300"});
            default :
                return setParams({ icon : process.env.PUBLIC_URL + `/images/Total.png` , name :"Total", amount : "$154,300" });
        }
    }
